...
typedef void proc_mossa ();
typedef proc_mossa * proc_mossa_ptr;
proc_mossa_ptr  history[2*MAXN];
...
void elenca_modi_ric(int n_i, int n_h, void pescato_intera(), void pescato_mezza(), void done() ) {
  ...
  if(n_i + n_h == 0) {
    ...
    history[pos_on_history++] = pescato_mezza;
    elenca_modi_ric(0, n_h-1, pescato_intera, pescato_mezza, done);
    pos_on_history--;
  }
  else if(n_h == 0) {
    history[pos_on_history++] = pescato_intera;
